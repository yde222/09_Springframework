...
* 통합 테스트
*
* 실제 스프링 컨텍스트를 로딩하여 MenuService의 전체 동작 (실제 DB와 연동 및 비즈니스 로직 적용)을
* 검증하는 테스트
* */
@SpringBootTest // 스프링부트의 전체 컨텍스트를 로딩
    ...
    public void testFindAllMenuByOrderableStatus_NotOrderable (){
        ...
        resultList.forEach(menu ->
                assertTrue(menu.getMenuName().endsWith(" (주문 불가능)"),
                        "주문 불가능 메뉴의 이름은 ' (주문 불가능)'으로 끝나야 한다,"));
