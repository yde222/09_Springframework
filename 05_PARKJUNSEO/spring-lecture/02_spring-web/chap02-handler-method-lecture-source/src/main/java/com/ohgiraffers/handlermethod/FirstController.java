...
public class FirstController {
    ...
    // first/regist로 보낸다.
    // 핸들러 메소드 반환값이 void인 경우 요청 주소가 곧 view의 이름이 된다.
    @GetMapping("/regist")
    public void regist() {
    }
    ...
    * */
    // WebRequest: HTTP 요청에 대한 정보를 추상화한 객체
    // Model: 컨트롤러에서 뷰(View)에 데이터를 전달할 때 사용하는 객체
    @PostMapping("/regist")
    public String registMenu(WebRequest request, Model model) {
        ...
        String message = name.trim() + "을(를) 신규 메뉴 목록의 " + categoryCode + "빈 카테고리에 " + price + "원으로 등록했습니다.";
    ...
    public String modifyMenu(
        ...
        String message = modifyName.trim() + " 메뉴의 가격을 " + modifyPrice + "원으로 변경했습니다.";
        model.addAttribute("message", message);
