...
class EntityLifeCycleTest {
    ...
    void testTransient(int menuCode) {
     //when
        Menu foundMenu = entityLifeCycle.findeMenuByMenuCode(menuCode);
    ...
    void testManagedOtherEntityManager(int menuCode) {
        // when
        Menu menu1 = entityLifeCycle.findeMenuByMenuCode(menuCode);
        System.out.println("menu1 = " + menu1.hashCode());
        Menu menu2 = entityLifeCycle.findeMenuByMenuCode(menuCode);
    ...
    void testDetachedEntity(int menuCode, int menuPrice) {
        ...
        // flush : 영속성 컨텍스트의 상태를 데이터베이스에 반영, 단 commit 하지 않았을 경우 rollback 가능
        entityManager.flush();
        // then
