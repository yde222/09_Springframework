...
public class SelectBuilderProvider {
    ...
    public String selectMenuByNameOrCategory(SearchCriteria creteria) {
        SQL sql = new SQL();
        sql.SELECT("menu_code", "menu_name", "menu_price", "category_code", "orderable_status")
           .FROM("tbl_menu");

        if ("category".equals(creteria.getCondition())) {
            sql.JOIN("tbl_category USING (category_code)")
               .WHERE("orderable_status = 'Y'")
                .AND()
                .WHERE("category_name = #{value}");
        }
        else if ("name".equals(creteria.getCondition())) {
            // where 메소드에 가변 인자로 전달하는 경우 자동적으로 AND 조건 처리되므로 OR의 경우 별도 메소드 사용
