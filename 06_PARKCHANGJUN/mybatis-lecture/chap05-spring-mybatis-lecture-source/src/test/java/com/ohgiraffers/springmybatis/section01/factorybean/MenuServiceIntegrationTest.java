...
/*
 * 통합 테스트
 * 실제 스프링 컨텍스트를 로딩하여 MenuService의 전체 동작(실제 DB와 연동 및 비즈니스 로직 적용)을 검증하는 테슽느
 */
...
@ActiveProfiles("test") // 테슽느 실행 시 application-test.yml 설정을 사용
@Transactional // 테스트 완료 후 DB에 반영 된 변경 사항을 롤백시키려는 용도로 사용
@Sql(statements = {
        // 테스트용 데이터 삽입
...
public class MenuServiceIntegrationTest {
    ...
    public void testFindAllMenuByOrderableStatus_Orderable() {
        ...
        resultList.forEach(menu -> assertTrue(menu.getMenuName().endsWith(" (주문 가능)"), "주문 가능 메뉴의 이름은 ' (주문 가능)'으로 끝나야 한다."));
    }
    ...
    public void testFindAllMenuByOrderableStatus_NotOrderable() {
        ...
        resultList.forEach(menu ->
                assertTrue(menu.getMenuName().endsWith(" (주문 불가능)"),
                        "주문 가능 메뉴의 이름은 ' (주문 불가능)'으로 끝나야 한다.")
        );
    }
}
