...
class EntityLifeCycleTest {
    ...
    void testDetachedEntity(int menuCode, int menuPrice) {
        ...
        entityTransaction.begin(); // 트랜잭션시작
    ...
    void testDetachAndMerge(int menuCode, int menuPrice) {
        ...
        entityManager.merge(foundMenu); // 새로운 데이터값이 들어감, 준영속 상태에서 다시 영속성 컨텍스트로 들어감
        entityManager.flush();
        // then
    ...
    // 11번행의 이름은 하양민트초코죽으로 변경
    void testMergeUpdate(int menuCode, String menuName) {
        EntityManager entityManager = EntityManagerGenerator.getInstance();
        Menu foundMenu = entityManager.find(Menu.class, menuCode); // 11번 행 : 정어리빙수
        entityManager.detach(foundMenu); // 준영속 -> 정어리 빙수가 떨어져나감
        ...
        // 마지막 값이 들어가는걸로 덮어쓰기 된다. 따라서 정어리빙수가 하양민트초코죽으로 값이 변한다.
    ...
    void testMergeSave() {
        ...
        entityTransaction.begin();
        foundMenu.setMenuName("치약맛 초코 아이스"); // 20, 마라깐쇼한라봉, 22000, 5, N --> 원래 값에서 999, 치약맛 초코 아이스, 22000, 5, N
    ...
    void testClearPersistenceContext(int menuCode) {
        ...
        assertNotEquals(foundMenu, expectedMenu); // 1과 3은 같지 않다 비교
    }
    ...
    void testRemoveEntity(int menuCode) {
        ...
        // 제거한다는것은 영속성 컨텍스트에서 빠진다고 생각하면 된다.
        entityManager.flush();
        // commit을 하지않았기 때문에 실행해도 지워지지 않는다.
