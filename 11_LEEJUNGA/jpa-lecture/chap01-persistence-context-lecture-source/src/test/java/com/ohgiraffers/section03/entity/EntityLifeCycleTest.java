...
class EntityLifeCycleTest {
    ...
    @DisplayName("다른 엔티티 매니저가 관리하는 엔터티의 영속성 테스트")
    @ParameterizedTest
    @ValueSource(ints = {1, 2})
    ...
    @ParameterizedTest
    @ValueSource(ints = {1, 2})
    void testManagedSamEntityManger(int menuCode) {
        // when
        EntityManager entityManager = entityLifeCycle.getManagerInstance();
    ...
    void testDetachedEntity(int menuCode, int menuPrice) {
        ...
        // detach : 특정 엔티티만 준영속 상(관리하던 객체를 관리하지 않는 상태로 변경)로 만듦
        ...
        // then
        assertNotEquals(menuPrice, entityManager.find(Menu.class, menuCode).getMenuCode());
    ...
    void testDetachAndMerge(int menuCode, int menuPrice) {
        ...
        // merge : 파라미터로 넘어온 준영속 엔티티 객체의 식별자 값으로 1차 캐시에서 엔터티 객체를 조회한다.
        // 없으면 Db에서 조회하여 1차 캐시에 저장한다.
        // 조회한 영속 엔터티 객체에 준영속 상태의 엔터티 객체의 값을 병합한 뒤 영속 센어티 객체를 반환한다.
    ...
    void testMergeUpdate(int menuCode, String menuName) {
        ...
        foundMenu.setMenuName(menuName); // 상태값 변경 완료 -> 상태변경변화는 없다.
    ...
    @DisplayName("dtach 후 merge 한 데이터 save 테스트")
    @Test
    void testMergeSave() {
        EntityManager entityManager = EntityManagerGenerator.getInstance();
        EntityTransaction entityTransaction = entityManager.getTransaction();
        ...
        entityManager.merge(foundMenu);
        entityTransaction.commit();  // 기존에 값이 없기 떄문에 insert문이 실행된다.
    ...
    void testClosePersistenceContext(int menuCode) {
        // close : 영속성 컨텍스트를 종료한다. -> 영속성 컨텍스트 내의 모든 엔터티는 준영속화 된다.
        EntityManager entityManager = EntityManagerGenerator.getInstance();
        entityManager.close();
    ...
    void testRemoveEntity(int menuCode) {
        ...
        // remove : 엔터리를 영속성 컨텍스트 및 데이터베이스에서 삭제한다.
        // 단, 트랜잭션을 제저하지 않으면 데이터베이스에 영구 반영되지는 않는다.
        entityManager.remove(foundMenu);
        entityManager.flush();
