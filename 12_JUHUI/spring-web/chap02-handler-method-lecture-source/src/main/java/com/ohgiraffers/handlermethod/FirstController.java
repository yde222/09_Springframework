...
public class FirstController {
    ...
    // /first/regist로 보낸다.
    // 즉, 핸들러 메서드 반환값이 void인 경우 요청 주소가 곧 view의 이름이 된다.
    @GetMapping("/regist")
    public void regist(){}
    ...
    * */
     ...
     * 전달하는 name 속성과 일치하는 것이 없는 경우 400 (Bad Request) 에러가 발생하는데(값이 잘못되었을 때 발생하는 오류)
     * 이는 required = true 가 기본 값이기 때문이다.
     * 값을 입력하지 않고 넘기면 빈 문자열이 넘어오므로 parsing 관련 에러가 발생할 수 있다.
     * */
    @PostMapping("/modify")
    public String modifyMenu(@RequestParam(value = "name", required = false) String modifyName, @RequestParam(value = "price", defaultValue = "0") int modifyPrice, Model model) {
        // @RequestParam은 parsing이 자동으로 됨.
