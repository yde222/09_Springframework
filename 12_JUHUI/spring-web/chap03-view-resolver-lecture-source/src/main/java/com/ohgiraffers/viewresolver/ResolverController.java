...
public class ResolverController {
    ...
    public String stringReturning(Model model){
        // Model : view에서 표현 되어야하는 동적인 데이털르 담는 용도로 사용하는 객체
        model.addAttribute("forwardMessage", "문자열로 뷰 이름 반환");

        // ViewResolver가 prefix/suffix를 합쳐서 물리적인 뷰를 선택
        return "result";
    }

    @GetMapping("/string-redirect")
    public String stringRedirect(){
        // 핸들러 메소드에서 viewersolver의 기본 전송방식은 forward.
        // 경우에 따라서 redirect로 전송을 하도록 해야하는데
        // 그때는 viewsolver 객체에게 redirect: 를 붙여서 요청하면
        // redirect 방식으로 이동
        return "redirect:/";
    }
     ...
     * 제거되게 할 수 있다.(그러니 이름은 같지 않게 하는 것이 필요함) */
