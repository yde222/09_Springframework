...
public class EntityLifeCycleTest {
    ...
    void testMergeUpdate(int menuCode, String menuName){
        EntityManager entityManager = EntityManagerGenerator.getInstance();
        Menu foundMenu = entityManager.find(Menu.class, menuCode); // 정어리 빙수
        entityManager.detach(foundMenu); // 준영속 상태 -> 정어리 빙수가 떨어져 나감.

        foundMenu.setMenuName(menuName); // 상태값 변경 완료 -> 상태 변경 변화는 없다. 정어리빙수 -> 하양민트초코죽
        Menu refoundMenu = entityManager.find(Menu.class, menuCode); // 다시 영속 상태를 만들어줌. 정어리빙수
        entityManager.merge(foundMenu);
    ...
    void testMergeSave(){
        ...
        entityManager.merge(foundMenu);
        entityTransaction.commit(); // 기존의 값이 없기 때문에 insert 문이 실행된다.
    ...
    void testClearPersistenceContext(int menuCode){
        ...
        Menu expectedMenu = entityManager.find(Menu.class, menuCode); // 쿼리문이 한 번 더 나간다는 뜻
        assertNotEquals(foundMenu, expectedMenu); // 같지 않다는 걸 통과시켜줌
    ...
    void testRemoveEntity(int menuCode){
        ...
        entityTransaction.begin();
        // remove : 엔터티를 영속성 켄텍스트 및 데이터베이스에서 삭제한다.
